...
export const getWalletBalance = async (network, provider, walletAddress) => {
  ...
  const referralsToClaim = Number(await walletContract._RefAmount(walletAddress)).toFixed(2);
  ...
  if (network === "bsc") {
    ...
    const referralsToClaim2 = Number(await walletContract5._RefAmount(walletAddress)).toFixed(2);
    ...
    const referralsToClaim2 = Number(await walletContract3._RefAmount(walletAddress)).toFixed(2);
  ...
  return {
    ...
    tokensToClaim: {
      amount: Number(tokensToClaim) === 8 ? 7.5 : tokensToClaim,
      value: Number(tokensToClaim) === 8 ? 7.5 : tokensToClaim,
    },
