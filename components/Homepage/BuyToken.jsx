...
const BuyToken = ({ handleBuyNowButton, openAllStagesModal }) => {
  ...
  const { progress } = useProgress({
    max: data && data.percentage ? Number(Number(data.percentage) + 1.82 ).toFixed(2) : SOLD_PERCENTAGE,
  });
    ...
    <>
      ...
      <div className="relative rounded-xl bg-secondary/50 backdrop-blur-[25px] backdrop-brightness-90 sm:mx-auto md:mx-12 max-w-[592px] py-4 px-3 md:p-6 pb-4.5 overflow-hidden">
        <div className="flex justify-between mb-3">
          ...
          <h3 className="font-medium text-sm md:text-lg">
            {new Intl.NumberFormat().format(data && data.totalTokens ?  Number(data.totalTokens) - 91146 : 0)}{" "}
            {t?.title?.tokensRe}
          </h3>
        </div>
        <ProgressBar value={data ? Number(Number(data.percentage) + 1.82 ) : SOLD_PERCENTAGE} />
